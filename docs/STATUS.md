...
- Served by Caddy from the same `web` image at `LANDING_DOMAIN` (default **http://localhost:8085** — plain HTTP locally so the bare URL works in a browser; a real domain in `LANDING_DOMAIN` gets automatic Let's Encrypt TLS. Dash/app stay HTTPS because PWA/camera require a secure context); `deploy/web.Dockerfile` builds all three frontends. Verified live alongside dash/app; desktop + mobile screenshots in `screens/landing-*.png`. (During verification: learned headless-Edge clamps window width ~500 px — the "mobile clipping" was the tool, not the page.)
