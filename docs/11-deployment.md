...
- [ ] Backups: nightly `docker compose exec db pg_dump -U owi owi | gzip > backup.sql.gz` + MinIO mirror (`mc mirror`) to a second location; run one restore drill per phase
